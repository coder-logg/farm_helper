...
begin
	if tg_table_name = 'plant' then
		...
		if (tg_op = 'UPDATE') then
			...
			amount_plant = new.amount;
			cost_plant = (SELECT cost FROM plant WHERE id = new.plant_id);
			cost_equipment_sum = (SELECT sum(cost) FROM Equipment
								  WHERE id = (
									  SELECT equipment_id FROM Required_equipment
									  WHERE plant_id = new.plant_id));
			UPDATE _order SET cost = amount_plant * cost_plant + cost_equipment_sum where order_detail_id = old.id;
		end if;
		return new;
	end if;
	return null;
...
CREATE OR REPLACE TRIGGER calc_order_cost
